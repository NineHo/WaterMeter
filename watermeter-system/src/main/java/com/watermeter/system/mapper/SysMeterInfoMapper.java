package com.watermeter.system.mapper;

import java.util.List;
import com.watermeter.system.domain.SysMeterInfo;
import com.watermeter.system.domain.SysDeviceData;

/**
 * 水表信息管理Mapper接口
 * 
 * @author Patrick
 * @date 2023-05-24
 */
public interface SysMeterInfoMapper 
{
    /**
     * 查询水表信息管理
     * 
     * @param id 水表信息管理主键
     * @return 水表信息管理
     */
    public SysMeterInfo selectSysMeterInfoById(Long id);

    /**
     * 查询水表信息管理列表
     * 
     * @param sysMeterInfo 水表信息管理
     * @return 水表信息管理集合
     */
    public List<SysMeterInfo> selectSysMeterInfoList(SysMeterInfo sysMeterInfo);

    /**
     * 新增水表信息管理
     * 
     * @param sysMeterInfo 水表信息管理
     * @return 结果
     */
    public int insertSysMeterInfo(SysMeterInfo sysMeterInfo);

    /**
     * 修改水表信息管理
     * 
     * @param sysMeterInfo 水表信息管理
     * @return 结果
     */
    public int updateSysMeterInfo(SysMeterInfo sysMeterInfo);

    /**
     * 删除水表信息管理
     * 
     * @param id 水表信息管理主键
     * @return 结果
     */
    public int deleteSysMeterInfoById(Long id);

    /**
     * 批量删除水表信息管理
     * 
     * @param ids 需要删除的数据主键集合
     * @return 结果
     */
    public int deleteSysMeterInfoByIds(String[] ids);

    /**
     * 批量删除设备数据
     * 
     * @param ids 需要删除的数据主键集合
     * @return 结果
     */
    public int deleteSysDeviceDataByDeviceIds(String[] ids);
    
    /**
     * 批量新增设备数据
     * 
     * @param sysDeviceDataList 设备数据列表
     * @return 结果
     */
    public int batchSysDeviceData(List<SysDeviceData> sysDeviceDataList);
    

    /**
     * 通过水表信息管理主键删除设备数据信息
     * 
     * @param id 水表信息管理ID
     * @return 结果
     */
    public int deleteSysDeviceDataByDeviceId(Long id);
}
